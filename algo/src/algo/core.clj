...
(defn sorted-insert
  "Insert a in a sorted sequence xs in such a way that f determines the min function
takes care of its placement"
  [f xs a]
  (let [[less more] (split-with #(f % a) xs)]
    (concat less [a] more)))

(defn insertion-sort
  "Sort the numbers with insertion sort"
  [f xs]
  (reduce #(sorted-insert f % %2) [] xs))

(defn selection-sort
  "Sorts the given elements in xs using selection sort with f giving comparison"
  [f xs]
...
(defn- count<=1?
  "Verify whether xs contains at least one element"
  [xs]
  (or (empty? xs) (empty? (next xs))))

(defn- bubble-iter
  "Represents one iteration of bubble sort"
  [f swapped? [x y & xs :as all]]
  (cond (count<=1? all) all
        (f x y) (cons x (bubble-iter f swapped? (cons y xs)))
        :else (cons y (bubble-iter f true (cons x xs)))))

(defn bubble-sort
  "Sorts the given array based on bubble sort"
  [f xs]
